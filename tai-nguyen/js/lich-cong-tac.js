...
function render(){
    ...
    if(coCanhBao && role!=="view" && alertSound){
...
function loadData(){
  fetch(API_URL)
    ...
    .catch(err => {
      console.error("Không tải được dữ liệu lịch công tác:", err);
    });
}

/* =====================================================
THUẬT TOÁN TÍNH ÂM LỊCH CHUẨN
===================================================== */
...
function convertSolar2Lunar(dd,mm,yy,timeZone){
  ...
  if(monthStart>dayNumber) monthStart=getNewMoonDay(k-1,timeZone);
...
function updateLichAm(){
  var elem = document.getElementById("lich-am");
  if(!elem) return;
  ...
  elem.innerHTML = "Âm lịch: <b>Ngày " + l[0] + " tháng " + l[1] + " năm " + l[2] + "</b>";
}
...
document.addEventListener("DOMContentLoaded", function() {
    if(ngayHienTai) ngayHienTai.innerText = hienNgayDayDu();
...
document.querySelectorAll(".header-nav a, .nav-links a").forEach(link => {
